...
// Configuration
const HF_ONNX_BASE = 'https://huggingface.co/KevinAHM/pocket-tts-onnx/resolve/main/onnx/';
const MODELS = {
    mimi_encoder: HF_ONNX_BASE + 'mimi_encoder.onnx',
    text_conditioner: HF_ONNX_BASE + 'text_conditioner.onnx',
    flow_lm_main: HF_ONNX_BASE + 'flow_lm_main_int8.onnx',
    flow_lm_flow: HF_ONNX_BASE + 'flow_lm_flow_int8.onnx',
    mimi_decoder: HF_ONNX_BASE + 'mimi_decoder_int8.onnx',
    tokenizer: './tokenizer.model',
    voices: './voices.bin'
};

// Smart model cache utility using Cache Storage (falls back to in-memory)
...
async function fetchAndCacheModel(url) {
    ...
    if (inMemoryModelCache.has(url)) {
        console.log('Model loaded from memory cache:', url);
        return inMemoryModelCache.get(url);
    }

    // Helper to try multiple URLs gracefully (e.g., HF -> local fallback)
    async function tryFetchAny(urls) {
        ...
        for (const u of urls) {
            try {
                ...
                if (r.ok) return r;
                lastErr = new Error(`Fetch failed ${u}: ${r.status} ${r.statusText}`);
            } catch (e) {
                lastErr = e;
            }
        }
    ...
    // Use Cache Storage if available
    if (typeof caches !== 'undefined') {
        try {
            const cache = await caches.open(MODEL_CACHE_NAME);
            const cached = await cache.match(url);
            if (cached) {
                console.log('Model loaded from CacheStorage:', url);
                const buf = await cached.arrayBuffer();
                inMemoryModelCache.set(url, buf);
                return buf;
            }

            // If URL is from HF, also try local fallback filename if network fails
            const tryUrls = [url];
            if (url.startsWith(HF_ONNX_BASE)) {
                const filename = url.substring(HF_ONNX_BASE.length);
                tryUrls.push('./onnx/' + filename);
            }

            console.log('Downloading model (tryUrls):', tryUrls);
            const res = await tryFetchAny(tryUrls);
            if (!res.ok) throw new Error(`Failed to download model ${url}: ${res.status} ${res.statusText}`);
            await cache.put(url, res.clone());
            const buf = await res.arrayBuffer();
            inMemoryModelCache.set(url, buf);
            return buf;
        } catch (e) {
            console.warn('CacheStorage unavailable or failed, falling back to fetch:', e);
        }
    }

    // Fallback: fetch directly (with local fallback for HF URLs) and save in memory cache
    console.log('Downloading model (no cache available):', url);
    const tryUrls2 = [url];
    if (url.startsWith(HF_ONNX_BASE)) {
        const filename = url.substring(HF_ONNX_BASE.length);
        tryUrls2.push('./onnx/' + filename);
    }
    const res2 = await tryFetchAny(tryUrls2);
    if (!res2.ok) throw new Error(`Failed to download model ${url}: ${res2.status} ${res2.statusText}`);
    const buf2 = await res2.arrayBuffer();
    inMemoryModelCache.set(url, buf2);
    return buf2;
}
    ...
    if (type === 'load') {
        ...
        try {
            if (typeof caches !== 'undefined') {
                ...
                postMessage({ type: 'cache_cleared', source: 'CacheStorage' });
            }
            inMemoryModelCache.clear();
            postMessage({ type: 'cache_cleared', source: 'memory' });
        } catch (err) {
            console.warn('Failed to clear model cache:', err);
            postMessage({ type: 'error', error: 'Failed to clear model cache: ' + err.toString() });
        }
...
async function loadModels() {
    ...
    try {
        ...
        // Load all models in parallel (with smart caching)
        postMessage({ type: 'status', status: 'Loading MIMI encoder...', state: 'loading' });
        console.log('Loading MIMI encoder...');

        // First download (or load from CacheStorage/in-memory) the model bytes in parallel
        const [encoderBuf, textCondBuf, flowMainBuf, flowFlowBuf, decoderBuf] = await Promise.all([
            fetchAndCacheModel(MODELS.mimi_encoder),
            fetchAndCacheModel(MODELS.text_conditioner),
            fetchAndCacheModel(MODELS.flow_lm_main),
            fetchAndCacheModel(MODELS.flow_lm_flow),
            fetchAndCacheModel(MODELS.mimi_decoder)
        ]);

        // Then create ONNX sessions from the ArrayBuffers (also in parallel)
        ...
        // Load tokenizer model (allow caching)
        const tokenizerBuffer = await fetchAndCacheModel(MODELS.tokenizer);
        ...
        try {
            // Load voices (allow caching)
            const voicesBuffer = await fetchAndCacheModel(MODELS.voices);
            if (voicesBuffer) {
                predefinedVoices = parseVoicesBin(voicesBuffer);
